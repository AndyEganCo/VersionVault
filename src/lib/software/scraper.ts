...
async function scrapeReleaseNotes(url: string): Promise<string> {
  try {
    ...
    const selectors = [
      ...
      '.releases',
      '.version-history',
      'section:contains("Release Notes")',
      'div:contains("Changelog")'
    ];
...
async function extractReleaseNotes(content: string, version: string): Promise<ReleaseNote> {
  try {
...
export async function getReleaseNotesForVersion(url: string, version: string): Promise<ReleaseNote> {
  const content = await scrapeReleaseNotes(url);
